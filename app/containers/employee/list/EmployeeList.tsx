...
const EmployeeList = (props: ScreenProps) => {
  ...
  const getData = useCallback(() => {
    ...
    return filterArray(query, [...props.getData], ['name']).sort(
      sortGetData(orderBy),
    );
  ...
  return (
    <Container style={Styles.container}>
      <Header title="Empleados" iconName="menu" leftIcon />
      <ContentFlatList
        data={getData()}
        //@ts-ignore
        renderItem={item => (
          <EmployeeListItem item={item.item} navigation={props.navigation} />
        )}
      ...
      <View style={Styles.buttonSection}>
        <TouchableHighlight
          ...
          <Ionicons name={'add-sharp'} style={Styles.createIcon} />
        </TouchableHighlight>
      </View>
      <ModalizeOrderBy
...
interface ScreenProps
  extends ReduxProps,
    StackScreenProps<DrawerNavigatorParamList, 'EmployeeList'> {}
