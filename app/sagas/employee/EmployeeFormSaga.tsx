...
import { LaborResponse, MasterResponse, PostEmployeeResponse } from '../../services/employee/EmployeeServiceConstants';
import ResponseCode from '../../services/ResponseCode';
import EmployeeFormActions, { Types as EmployeeFormTypes } from '../../stores/employee/form/Actions';

function* getEmployeeForm(request: any) {
  yield put(EmployeeFormActions.getEmployeeFormLoading());

  const [laborList, masterList]: [ApiResponse<LaborResponse>, ApiResponse<MasterResponse>] = yield all([
  ...
  if (!laborList || !masterList) {
    yield put(EmployeeFormActions.getEmployeeFormFailure(ResponseCode.BAD_REQUEST));
    return
  }
...
function* postEmployeeForm(request: any) {
  ...
  const response: ApiResponse<PostEmployeeResponse> = yield call(EmployeeService.postForm, request.payload?.apiRequest);

  if (!response) {
    yield put(EmployeeFormActions.postEmployeeFormFailure(ResponseCode.BAD_REQUEST));
    return
  }
  ...
  const luxandResponse: GoogleResponse<any> = yield call(EmployeeService.uploadPhoto, luxandRequest)

  if (!luxandResponse || luxandResponse.fail) {
    yield put(EmployeeFormActions.postEmployeeFormFailure({
      code: 500,
